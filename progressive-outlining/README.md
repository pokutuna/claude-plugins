...
brief と案は一続きで書き、最初の止まり所は案の選択。以降は成果をファイルに書くたびに止まり、チャットには何を書いたかと推奨する次の一手だけを短く返す。
ユーザーはファイルを読んでから返答し、その内容で次が決まる。

- マーカーや直接編集を入れて反映を求める: 同じ段階をもう一往復
- 次へ: 次の段階の成果を出して止まる
- 自走: 批評役 SubAgent と往復して進める (writing は次の段階だけ、または完成まで。coding は残り全部)
- coding では、ここで設計を終えて実装へ引き渡す

AskUserQuestion は brief の不足項目と案の選択にだけ使う。案の選択はユーザー、以降の展開は brief を基準にした自走、という配分を既定で推奨する。
自走の後でも、作業ファイルにコメントを書けば任意の段階に戻って直せる。Agent ツールがない環境では自走は案内しない。

## 作業ファイルとフィードバック記法

往復は 1 つの作業ファイルで行い、長い内容はチャットに貼らずファイルで受け渡す。writing は `<出力名>.outline.md`、coding は設計文書そのもの。
